...
public class GeometryVisitorTest {
	...
	public void TestVisitor() throws UnsupportedEncodingException {
		...
		GeometryCollection geometryCollection = new GeometryCollection(
				Arrays.asList(new Point(new Coordinate(1.0, 2.0)), new Point(new Coordinate(3.0, 4.0))));
	...
	public void testEnvelopeBuilder() {
		...
		LineString lineString = new LineString(
				Arrays.asList(new Point(new Coordinate(3.0, 4.0)), new Point(new Coordinate(5.0, 6.0))));
		GeometryCollection geometryCollection = new GeometryCollection(Arrays.asList(point, lineString));

		EnvelopeBuilder envelopeBuilder = new EnvelopeBuilder();
		geometryCollection.accept(envelopeBuilder);

		Envelope result = envelopeBuilder.build();
		assertEquals(1.0, result.getXmin(), EPSILON);
		assertEquals(2.0, result.getYmin(), EPSILON);
		assertEquals(5.0, result.getXmax(), EPSILON);
		assertEquals(6.0, result.getYmax(), EPSILON);
	}
	...
	public void testWktVisitor() {
		...
		String result = wktVisitor.getResult();
		assertEquals("GEOMETRYCOLLECTION(POINT(1.0 2.0),LINESTRING(3.0 4.0,5.0 6.0))", result);
	}
	...
	public void testLengthVisitor() {
		...
		geometryCollection.accept(lengthVisitor);

		double result = lengthVisitor.getTotalLength();
